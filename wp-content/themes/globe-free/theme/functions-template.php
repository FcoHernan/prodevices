...
if ( ! function_exists( 'yit_button_style' ) ) {
    ...
    function yit_button_style( $button = array() ) {
        $button['flat']        = __( 'Flat Button', 'yit' );
        $button['ghost']       = __( 'Ghost Button', 'yit' );
        $button['ghost-white'] = __( 'Ghost White Button', 'yit' );
        ...
        if ( $type == 'square' ) {
            ...
            foreach ( $socials as $i => $social ) {
                ...
                switch( $social ){
                    ...
                    case 'envelope':
                        $url = apply_filters( 'yit_share_mail', 'mailto:?subject=I wanted you to see this site&amp;body= ' . $permalink . '&amp;title=' . $title . '' );
                        break;
                } ?>

                <li>
                    <a href='<?php echo $url; ?>' class="social-square <?php echo $social ?>" target="_blank" <?php echo $attrs ?>>
                        <?php if( $show_text ) : ?>
                            <?php _e( 'Share on ', 'yit' ); ?>
                            <?php echo $social; ?>
                        <?php endif; ?>
    ...
    function yit_testimonial_add_fields(){
        ...
        return array(
            'yit_testimonial_role'      => array(
                'label' => __( 'Role', 'yit' ),
                'desc'  => __( 'Insert the role used for testimonial', 'yit' ),
                'type'  => 'text',
                'std'   => '' ),
