...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        protected function __construct() {
            ...
            $this->_marketplace = YIT_MARKETPLACE == 'tf' ? 'ThemeForest' : 'Yithemes';
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( $this->is_automatic ) {
                    ...
                    if ( is_wp_error( $activate ) ) {
                        ...
                        echo '<p><a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( 'themes.php' ) ) ) . '" target="_parent">' . $this->strings['return'] . '</a></p>';
                ...
                foreach ( $this->plugins as $plugin ) {
                    if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                        echo '<p><a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '" target="_parent">' . $this->strings['return'] . '</a></p>';
                        $complete[] = $plugin;
                        break;
                    }
                    // Nothing to store.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' . sprintf( $this->strings['complete'], '<a href="' . esc_url( self_admin_url() ) . '">' . __( 'Return to the Dashboard', 'tgmpa' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                if ( $action_links ) {
                    $rendered .= apply_filters( 'tgmpa_notice_rendered_action_links', '<p class="action_links">' . implode( ' | ', $action_links ) . '</p>' );
                }
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
    ...
    class TGMPA_List_Table extends WP_List_Table {
        ...
        public function __construct() {
            $this->admin_page_base = $GLOBALS['tgmpa']::$instance->parent_slug;
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[ $item['file_path'] ] ) ) {
                ...
                if ( 'Pre-Packaged' == $item['source'] && ! file_exists( $item['url'] ) ) {
                    ...
                    $actions = array(
                        'install' => '<span style="font-size: 12px; color: #aaa;">' . __( $msg, 'yit' ) . '</span>',
                    );
                } else {
        ...
        public function column_cb( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) && 'Pre-Packaged' === $item['source'] ) {
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'tgmpa' ), self_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() ) {
                ...
                $url   = esc_url_raw( add_query_arg( array( 'page' => $GLOBALS['tgmpa']::$instance->menu ), self_admin_url( $this->admin_page_base ) ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
            ...
            if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function bulk_footer() {
                        ...
                        foreach ( $GLOBALS['tgmpa']::$instance->plugins as $plugin ) {
                            if ( ! is_plugin_active( $plugin['file_path'] ) ) {
                                echo '<p><a href="' . esc_url( add_query_arg( 'page', $GLOBALS['tgmpa']::$instance->menu, self_admin_url( $GLOBALS['tgmpa']::$instance->parent_slug ) ) ) . '" target="_parent">' . $GLOBALS['tgmpa']::$instance->strings['return'] . '</a></p>';
                                $complete[] = $plugin;
                                break;
                            }
                            // Nothing to store.
